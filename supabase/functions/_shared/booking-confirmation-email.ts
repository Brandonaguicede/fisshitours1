...
export async function buildBookingRequestCustomerHtml(supabase: SupabaseClient, bookingId: string): Promise<string> {
  const { data: booking, error } = await supabase.from('bookings')
    .select('booking_reference, tour_date, guests, total_snapshot, customers(full_name), boats(name), tours(title), tour_packages(name), time_slots(label)')
...
function buildCustomerHtml(input: { name: string; reference: string; date: string; time: string; tour?: string | null; boat?: string | null; packageName?: string | null; guests: number; total: string; paymentStatus: string; whatsappNumber: string; heading?: string; introduction?: string }) {
  const e = escapeHtml;
