...
async function getBookingConfirmationMessages(supabase: SupabaseClient, bookingId: string): Promise<ConfirmationMessage[]> {
  ...
  const customerText = `Hola ${booking.customers.full_name},\n\nTu pago de PayPal fue recibido correctamente. En unos momentos recibirÃ¡s la confirmaciÃ³n de tu reserva. Muchas gracias por reservar con Papagayo Fishing Tours.\n\n${summary}\n\nLlega a la marina 15 minutos antes de la salida.\n\nWhatsApp: https://wa.me/${whatsappNumber}\n\nPura Vida,\nPapagayo Fishing Tours`;
...
function buildCustomerHtml(input: { name: string; reference: string; date: string; time: string; tour?: string | null; boat?: string | null; packageName?: string | null; guests: number; total: string; paymentStatus: string; whatsappNumber: string }) {
  ...
  return `<!doctype html><html><body style="margin:0;background:#f1f6f8;color:#0f2742;font-family:Arial,Helvetica,sans-serif"><table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="background:#f1f6f8"><tr><td align="center" style="padding:24px 12px"><table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="max-width:620px;background:#fff;border-radius:16px;overflow:hidden"><tr><td align="center" style="background:#082c4c;padding:28px 20px"><img src="https://papagayofishingtours.com/images/papagayo-logo.png" width="260" alt="Papagayo Fishing Tours" style="display:block;width:260px;max-width:90%;height:auto;border:0"></td></tr><tr><td style="padding:34px 30px 16px"><div style="color:#159a72;font-size:28px;font-weight:700;text-align:center">âœ“</div><h1 style="margin:8px 0 12px;text-align:center;color:#0f2742;font-size:26px">Pago recibido y reserva confirmada</h1><p style="margin:0;text-align:center;color:#475569;font-size:15px;line-height:1.6">Tu pago fue recibido correctamente.<br>Muchas gracias por reservar con Papagayo Fishing Tours.</p><p style="margin:22px 0 0;color:#0f2742;font-size:16px">Hola ${e(input.name)},</p></td></tr><tr><td style="padding:0 30px 18px"><table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="background:#eaf4f8;border-radius:12px"><tr><td style="padding:18px 20px;width:50%;vertical-align:top"><div style="color:#64748b;font-size:12px">Referencia de reserva</div><div style="margin-top:5px;color:#0f2742;font-size:17px;font-weight:700">${e(input.reference)}</div></td><td style="padding:18px 20px;width:50%;vertical-align:top"><div style="color:#64748b;font-size:12px">Fecha del tour</div><div style="margin-top:5px;color:#0f2742;font-size:15px;font-weight:700">${e(input.date)}</div><div style="margin-top:4px;color:#0f2742;font-size:14px">${e(input.time || '-')}</div></td></tr></table></td></tr><tr><td style="padding:0 30px 18px"><h2 style="margin:8px 0 10px;color:#0f2742;font-size:17px">Detalles de la reserva</h2><table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="border-top:1px solid #e2e8f0">${row('Tour', input.tour)}${row('Boat', input.boat)}${row('Package', input.packageName)}${row('Guests', input.guests)}</table></td></tr><tr><td style="padding:0 30px 18px"><h2 style="margin:8px 0 10px;color:#0f2742;font-size:17px">Resumen del pago</h2><table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="border-top:1px solid #e2e8f0">${row('Total', input.total)}${row('Estado del pago', input.paymentStatus)}</table></td></tr><tr><td style="padding:0 30px 18px"><table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="background:#f3f8f6;border-radius:12px"><tr><td style="padding:18px 20px"><h2 style="margin:0 0 8px;color:#0f2742;font-size:16px">Información importante</h2><p style="margin:0;color:#475569;font-size:14px;line-height:1.6">Llega a la marina 15 minutos antes de la salida.<br><br>Si tienes preguntas o necesitas hacer cambios, contáctanos.</p></td></tr></table></td></tr><tr><td align="center" style="padding:4px 30px 28px"><a href="https://wa.me/${e(input.whatsappNumber)}" style="display:inline-block;background:#159a72;color:#fff;text-decoration:none;border-radius:8px;padding:14px 24px;font-size:15px;font-weight:700">Contáctanos por WhatsApp</a></td></tr><tr><td style="padding:0 30px 28px;color:#475569;font-size:14px;line-height:1.6">Estamos listos para ofrecerte una experiencia increíble.<br><br>Pura Vida,<br><strong style="color:#0f2742">Papagayo Fishing Tours</strong></td></tr><tr><td align="center" style="background:#082c4c;padding:18px;color:#dbeafe;font-size:12px;line-height:1.6">Papagayo Fishing Tours<br>Costa Rica<br><a href="https://papagayofishingtourcr.com" style="color:#dbeafe">papagayofishingtourcr.com</a></td></tr></table></td></tr></table></body></html>`;
}
