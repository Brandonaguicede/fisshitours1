...
async function getBookingConfirmationMessages(supabase: SupabaseClient, bookingId: string): Promise<ConfirmationMessage[]> {
  ...
  const meetingPointText = booking.departure_location_name_snapshot?.trim() || 'el punto de encuentro';
  ...
  const messages: ConfirmationMessage[] = [{ to: booking.customers.email, subject: `Pago recibido y reserva confirmada - ${booking.booking_reference}`, text: customerText, html: buildBookingHtml({ name: booking.customers.full_name, reference: booking.booking_reference, date: dateLabel, time: timeLabel, tour: booking.tours?.title, boat: booking.boats?.name, packageName, guests: booking.guests, departureLocation: booking.departure_location_name_snapshot, total: formatUsd(Number(booking.total_snapshot ?? 0)), paymentStatus: paid ? 'Paid' : 'Pending', whatsappNumber }), dedupe: `booking:${booking.id}:paypal-confirmation-customer-email` }];
  if (adminEmail) {
...
function buildBookingHtml(input: { name: string; reference: string; date: string; time: string; tour?: string | null; boat?: string | null; packageName?: string | null; guests: number; departureLocation?: string | null; total: string; paymentStatus: string; whatsappNumber: string; heading?: string; introduction?: string; contact?: { email: string; whatsapp: string } | null; ctaLabel?: string }) {
  ...
  const meetingPoint = input.departureLocation?.trim() || 'el punto de encuentro';
