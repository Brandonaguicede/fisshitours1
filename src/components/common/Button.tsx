...
const variants: Record<ButtonVariant, string> = {
  primary: 'bg-ocean-500 text-ocean-950 shadow-soft hover:-translate-y-0.5 hover:bg-ocean-400 hover:shadow-lifted',
...
export function Button({ children, className, variant = 'primary', to, type = 'button', ...props }: ButtonProps) {
  const classes = cn(
    'focus-ring pressable inline-flex min-h-11 items-center justify-center rounded-full px-5 py-2.5 text-sm font-semibold',
    variants[variant],
    className,
  );
