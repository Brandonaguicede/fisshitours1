  ...
  return (
    ...
    >
      <Container className="flex h-20 items-center justify-between">
        <NavLink
          className="focus-ring flex items-center gap-4 rounded-full text-white transition-colors duration-300"
          to="/"
          onClick={() => setIsOpen(false)}
        >
          <span className={cn('grid h-28 w-28 place-items-center transition-all duration-300', isScrolled ? 'drop-shadow-lg' : 'drop-shadow-sm')}>
        ...
        <button
          className={cn(
            'focus-ring pressable inline-flex h-11 w-11 items-center justify-center rounded-full border transition-all duration-300 md:hidden',
            isScrolled ? 'border-white/15 bg-white/10 text-white shadow-sm backdrop-blur-xl' : 'border-white/35 bg-white/10 text-white backdrop-blur-xl',
          )}
          type="button"
        ...
        <div className="border-t border-white/10 bg-ocean-950/96 shadow-soft backdrop-blur-2xl md:hidden">
          <Container className="grid gap-3 py-4">
            {navigationItems.map((item) => (
              <NavLink
                key={item.href}
                className="focus-ring rounded-2xl px-4 py-3 font-semibold text-white hover:bg-white hover:text-ocean-900"
                to={item.href}
                onClick={() => setIsOpen(false)}
              >
                {navLabels[item.href]?.[language] ?? item.label}
            ...
            <button
              className="focus-ring pressable rounded-2xl border border-white/15 bg-white/10 px-4 py-3 text-left font-semibold text-white hover:bg-white hover:text-ocean-900"
              type="button"
              aria-label={language === 'es' ? 'Cambiar a ingles' : 'Switch to Spanish'}
              onClick={toggleLanguage}
            >
