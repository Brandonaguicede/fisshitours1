  ...
  return (
    <article className="glass-card group overflow-hidden rounded-2xl transition duration-300 hover:-translate-y-1 hover:border-ocean-500 hover:shadow-lifted">
      <div className="relative overflow-hidden">
        <img className="aspect-[4/3] w-full object-cover transition duration-700 ease-out group-hover:scale-105" src={tour.image} alt={title} loading="lazy" />
        <div className="absolute inset-x-0 bottom-0 h-28 bg-gradient-to-t from-ocean-950/65 to-transparent" />
        <span className="absolute left-4 top-4 rounded-full bg-ocean-950/75 px-3 py-1 text-xs font-bold text-ocean-100 backdrop-blur">{category}</span>
      </div>
      ...
      <div className="p-5">
        ...
        <p className="mt-2 line-clamp-2 text-sm leading-6 text-ocean-200">{description}</p>
        {duration ? (
          <span className="mt-4 flex items-center gap-2 text-sm text-ocean-200">
            <Clock size={16} className="text-ocean-600" /> {duration}
          </span>
        ) : null}
