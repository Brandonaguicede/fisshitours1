...
test('Portada is a Hero-only screen: Media / Textos / Imágenes / Video, no About tab or field, and no "Hero" label anywhere', async () => {
  ...
  try {
    ...
    await expect(page.getByLabel(/^Historia/)).toHaveCount(0);
...
test('Sobre Nosotros keeps every About field, image and alt text, and has no Hero controls', async () => {
  ...
  try {
    ...
    for (const label of ['Titulo', /^Descripcion/, /^Texto de inicio/, /^Historia/, 'Texto alternativo imagen']) {
    ...
    await expect(page.locator('.admin-media-card img[src="https://example.test/site-images/general/abc/one.jpg"]')).toHaveCount(1);
    await expect(page.locator('.admin-media-card img[src="https://example.test/site-images/general/abc/five.jpg"]')).toHaveCount(1);
...
const ABOUT_TEXT_KEYS = ['about.title', 'about.description', 'about.preview_text', 'about.story', 'about.image_alt'];
...
test('Sobre Nosotros preserves the complete About key inventory: 7 editable texts (EN saved, ES generated) and nothing extra', async () => {
  ...
  try {
    ...
    const fields = [
      ...
      [page.getByLabel(/^Texto de inicio/), 'about.preview_text', 'P1\n\nP2'],
      [page.getByLabel(/^Historia/), 'about.story', 'S1\n\nS2'],
  ...
  try {
    ...
    for (const theme of ['light', 'dark']) {
      ...
      const preview = await page.evaluate(() => {
        ...
        const eyebrow = getComputedStyle(document.querySelector('.admin-hero-preview__eyebrow'));
        const lead = getComputedStyle(document.querySelector('.admin-hero-preview__subtitle'));
